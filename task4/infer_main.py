...
def load_model_and_tokenizer(model_dir, trust_remote_code = True):
    ...
    for t in range(assure_time):
        ...
        for sentence in tqdm(sentences_ls,desc=f'assure {t+1:03d}'):
            response, history = model.chat(tokenizer, 
                                           ...
                                           history=history, 
                                           do_sample=True,
                                           num_beams=1, top_p=0.8, temperature=0.2)
...
if __name__=='__main__':
    ...
    parser.add_argument('--model_path', type=str, default='/data/hzy/ChatGLM3/models/pv2_model/checkpoint-400')   
    args = parser.parse_args()

    data_dir = '/data/hzy/ChatGLM3/task4/dataset'
    save_dir = os.path.join(sys.path[0],'Result')
    os.makedirs(save_dir,exist_ok=True)

    cls_list_path = os.path.join(data_dir,'cls.txt')
    if os.path.exists(cls_list_path):
        ...
        example = {}
        for cls in cls_list:
            cls_info = pd.read_csv(os.path.join(data_dir,f'{cls}.csv'))
            example[cls] = cls_info['Sentence'].tolist()

        val_data = pd.read_csv(os.path.join(data_dir, "val.csv"))
        test_data = pd.read_csv(os.path.join(data_dir, "test.csv"))
    else:
        example, val_data, test_data = train_val_split(os.path.join(data_dir,'origin'))
        cls_list = list(example.keys())
    
    model, tokenizer = load_model_and_tokenizer(args.model_path)

    sys_prompt = init_prompt(example, pre_hist_rate=0.2, shot=5)
    ...
    refer_data = pd.read_csv(os.path.join(data_dir,'refer.csv'))
    test_acc = 0.97229*np.sum(test_data['Category']==refer_data['Category'])/len(test_data)
    print(f'test_acc: {test_acc}')
    ft_name = args.model_path.split(os.path.sep)[-2].split('_')[0]
    test_data.to_csv(os.path.join(save_dir,f'{ft_name}_val({val_acc:.2f})_test({test_acc:.2f}).csv'),index=False)